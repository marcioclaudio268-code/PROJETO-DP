...
def test_mapping_pipeline_persists_mapped_output(tmp_path: Path):
    ...
    artifacts = map_snapshot_with_company_config(snapshot_path, config_path)

    assert artifacts.output_path.exists()

    payload = json.loads(artifacts.output_path.read_text(encoding="utf-8"))
    assert payload["artifact_version"] == "mapping_result_v1"
    assert payload["config"]["config_version"] == "2024.03.01"
    assert payload["mapped_movements"][0]["resolved_domain_registration"] == "123"
    assert payload["mapped_movements"][0]["output_rubric"] == "201"
    assert payload["counts"]["ready_movements"] == 1
